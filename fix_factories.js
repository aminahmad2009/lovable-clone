...
// Step 1: Replace the skills factory section (from the comment to the end of getSkills function) with new local repository based functions.
...
const newContent = content.substring(0, startIndex) + newGetSkillsFunction + content.substring(braceAbsoluteIndex + 1);
content = newContent;
...
// Step 3: Insert the MCP factory functions after the getSkills function (which is now the new one) and before the skills factory routes? 
// Actually, we want to keep the MCP factory functions near the skills factory functions. Let's put them after the getSkills function and before the skills factory routes.
// But note: we have already inserted the skills factory routes after the skills section. The getSkills function is before the skills section.
// So we can insert the MCP factory functions between the getSkills function and the skills section.

// We'll find the end of the getSkills function again in the updated content.
const getSkillsIndex = content.indexOf('async function getSkills()');
if (getSkillsIndex === -1) {
  console.error('ERROR: Could not find the getSkills function');
  process.exit(1);
}

// Find the end of the getSkills function.
braceCount = 0;
inRoute = false;
i = getSkillsIndex;
while (i < content.length) {
  if (content.substring(i, i + 12) === 'async function') {
    inRoute = true;
    i += 12;
    continue;
  }
  if (inRoute) {
    if (content[i] === '{') braceCount++;
    if (content[i] === '}') braceCount--;
    if (braceCount === 0 && i > getSkillsIndex) {
      // We found the end of the function
      const getSkillsEndIndex = i; // index of the closing brace
      // Now, we want to insert the MCP factory functions after this closing brace and before the skills section.
      // Find the start of the skills section (the comment /* skills */ or the route('GET', '/api/designs'))
      const skillsSectionStartIndex = content.indexOf('/* skills */', getSkillsEndIndex);
      if (skillsSectionStartIndex === -1) {
        console.error('ERROR: Could not find the skills section start');
        process.exit(1);
      }
      // Now, we can insert the MCP factory functions between the getSkills function and the skills section start.
      const mcpFactoryFunctions = `
...
`;
      ...
      // Insert the MCP factory functions after the getSkills function and before the skills section.
      content = content.substring(0, getSkillsEndIndex + 1) + mcpFactoryFunctions + content.substring(getSkillsEndIndex + 1);
      // Now, we need to update the insertAfterIndex for the skills factory routes because we have added content before it.
      // The skills factory routes were inserted after the skills section end, but we have added content before the skills section.
      // So we need to adjust the insertAfterIndex by the length of the inserted MCP factory functions.
      insertAfterIndex += mcpFactoryFunctions.length;
      break;
    }
  }
  i++;
...
// Step 4: Insert the MCP factory routes after the skills factory routes.
...
while (i < content.length) {
  ...
  if (inRoute) {
    if (content[i] === '{') braceCount++;
    if (content[i] === '}') braceCount--;
    if (braceCount === 0 && i > leaderboardIndex) {
      // We found the end of the route handler function
      // Now, find the end of the route statement (semicolon)
      let j = i + 1;
      while (j < content.length && /\s/.test(content[j])) {
      ...
      if (j < content.length && content[j] === ';') {
        // We found the semicolon
        const leaderboardEndIndex = j; // index of the semicolon
