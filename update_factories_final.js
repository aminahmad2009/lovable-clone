...
// Step 2: Insert the MCP factory functions (loadMcpServersFromLocalRepo and getMcpServers) right after the getSkills function (which is now the new one) and before the MIME constant.
...
while (i < content.length) {
  ...
  if (inFunction) {
    ...
    if (braceCount === 0 && i > getSkillsIndex) {
      ...
      // Now, we want to insert the MCP factory functions after this closing brace and before the MIME constant.
      // Find the start of the MIME constant.
      const mimeIndex = content.indexOf('const MIME = {', getSkillsEndIndex);
      if (mimeIndex === -1) {
        console.error('ERROR: Could not find the MIME constant');
        process.exit(1);
      }
      // Now, we can insert the MCP factory functions between the getSkills function and the MIME constant.
...
`;
      ...
      // Insert the MCP factory functions after the getSkills function and before the MIME constant.
...
// Step 3: Insert the skills factory routes after the skills section (after the DELETE /api/skills/:skillId route).
...
braceCount = 0;
let inRoute = false;
let j = skillsSectionEndIndex;
while (j < content.length) {
  if (content.substring(j, j + 12) === '=> {') {
    inRoute = true;
    j += 12;
    continue;
  }
  if (inRoute) {
    ...
    if (content[j] === '}') braceCount--;
    if (braceCount === 0 && j > skillsSectionEndIndex) {
      ...
      if (k < content.length && content[k] === ';') {
        ...
        skillsSectionEndIndex = k; // update to the semicolon
      }
      break;
    }
  }
